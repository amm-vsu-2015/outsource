...
private:
  ...
  void equalMonthDay() { // проверка равенства числа и месяца
    if (dd == mm) {
      cout << "Число дня и месяца равны" << endl;
    } else {
      cout << "Число дня и месяца НЕ равны" << endl;
    }
  }
...
int main() {
  ...
  myDate.equalMonthDay();

  myDate.increaseMonth();
  std::cout << "Прибавили 1 месяц: " << myDate.toString()  << std::endl;

  //текущая дата
  //Date nowDate = Date();
  //std::cout << "Текущая дата: " << nowDate.toString() << std::endl;
